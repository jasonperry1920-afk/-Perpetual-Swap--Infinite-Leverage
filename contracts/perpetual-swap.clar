...
(define-read-only (calculate-position-value (position-size uint) (current-price uint))
...
(define-read-only (calculate-pnl (user principal))
    ...
    (if is-long
        (ok (if (>= current-price entry-price)
            ...
            (- 0 (to-int (/ (* position-size (- entry-price current-price)) PRECISION)))
        ))
        (ok (if (>= entry-price current-price)
            (to-int (/ (* position-size (- entry-price current-price)) PRECISION))
            (- 0 (to-int (/ (* position-size (- current-price entry-price)) PRECISION)))
        ))
    ))
)
...
(define-read-only (calculate-margin-ratio (user principal))
    (let (
        ...
        (position-value (calculate-position-value (get position-size position) (var-get stx-price)))
    )
    (if (> pnl 0)
        (ok (/ (* (+ collateral (to-uint pnl)) PRECISION) position-value))
        (ok (if (>= collateral (to-uint (- 0 pnl)))
            (/ (* (- collateral (to-uint (- 0 pnl))) PRECISION) position-value)
            u0
        ))
    ))
)
...
(define-read-only (is-liquidatable (user principal))
    (let (
        (margin-ratio-result (calculate-margin-ratio user))
    )
...
(define-public (withdraw (amount uint))
    ...
    (asserts! (is-none (map-get? positions tx-sender)) ERR_ALREADY_HAS_POSITION)
    (map-set user-balances tx-sender (- current-balance amount))
    (try! (as-contract (stx-transfer? amount tx-sender tx-sender)))
    (ok true))
)

(define-public (open-position (collateral-amount uint) (position-size uint) (is-long bool))
    (let (
        (current-balance (get-user-balance tx-sender))
        (current-price (var-get stx-price))
    )
    ...
    (if is-long
        (var-set total-long-positions (+ (var-get total-long-positions) position-size))
...
(define-public (liquidate (user principal))
    ...
    (map-set user-balances tx-sender (+ liquidator-balance liquidation-reward))
    (if is-long
        (var-set total-long-positions (- (var-get total-long-positions) position-size))
        (var-set total-short-positions (- (var-get total-short-positions) position-size))
    )
...
(define-public (apply-funding-payment (user principal))
    ...
    (if (> blocks-elapsed u0)
        (let (
            (payment-calc (/ (* (if (> funding 0) (to-uint funding) (to-uint (- 0 funding))) position-size blocks-elapsed) FUNDING_RATE_DIVISOR))
            (collateral (get collateral position))
            (should-deduct (or (and is-long (> funding 0)) (and (not is-long) (< funding 0))))
            (new-collateral (if should-deduct
                (if (>= collateral payment-calc) (- collateral payment-calc) u0)
                (+ collateral payment-calc)
            ))
        )
        (map-set positions user (merge position {
